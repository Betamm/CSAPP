...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges) 
{
	...
	size = trace->ops[i].size;
	     ...
	     */
	    //printf("alloc, memset(p, index & 0xFF, size), memset(%p, %d, %d)\n", p, index & 0xFF, size);
	    ...
	    for (j = 0; j < oldsize; j++) {
		  ...
		  printf("error j: %d\n", j);
		  printf("index: %d\n", index);
		  printf("newp[j-2] - newp[j+2]: %d, %d, %d, %d, %d, %d, %d, %d\n", newp[j-2], newp[j-1], newp[j], newp[j+1], newp[j+2], newp[j+3], newp[j+4], newp[j+5]);
		  printf("newp[520] - newp[526]: %d, %d, %d, %d, %d, %d, %d, %d\n", newp[631], newp[632], newp[633], newp[634], newp[635], newp[636], newp[637], newp[638]);
		  for (int i = 631; i < 646; i++){
		    printf("%d  ", newp[i]);
		  }
		  printf("\n");
		  for (int i = 631; i < 646; i++){
	    ...
	    //printf("realloc, memset(newp, index & 0xFF, size), memset(%p, %d, %d)\n", newp, index & 0xFF, size);
...
{   
	...
	//printf("\n@@@%d :  %d ---- %f\n", i, total_size, (double)mem_heapsize());
    }
    //printf("\n***: %f, %f ***\n",(double)(max_total_size), (double)mem_heapsize());
